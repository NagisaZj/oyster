...
from rlkit.core.rl_algorithm import MetaRLAlgorithm, ExpAlgorithm,ExpAlgorithmSimple,ExpAlgorithmSimpleOnline,ExpAlgorithmIter
...
class PEARLSoftActorCritic(MetaRLAlgorithm):
    def __init__(
        ...
        self.use_info_in_context = use_info_in_context
    ...
    def sample_context(self, indices):
        ...
        # full context consists of [obs, act, rewards, next_obs, terms, info]
        ...
        if self.use_info_in_context:
            context = torch.cat([context,tmp[-1]],dim=2)
        return context
    ...
    def _take_step(self, indices, context):
        ...
        #print(obs[:1,:5,:],actions[:1,:5,:],rewards[:1,:5,:],next_obs[:1,:5,:])
...
class ExpSAC(ExpAlgorithm):
    ...
    def _min_q_exp(self, obs, actions, z_mean, z_var):
        ...
        q1 = self.qf1_exp(obs, actions, z_mean, z_var)
        q2 = self.qf2_exp(obs, actions, z_mean, z_var)
        min_q = torch.min(q1, q2)
        return min_q
    ...
    def _take_step(self, indices, context,context_unbatched):
        ...
        #encoder_next = encoder_output
        #encoder_output = self.duiqi(encoder_output)
        # flattens out the task dimension
        t, b, _ = obs.size()
        ...
        with torch.no_grad():
            target_v_values = self.target_exp_vf(next_obs, z_mean_next,z_var_next)
...
class ExpSACSimple(ExpAlgorithmSimple):
    def __init__(
            self,
            env,
            train_tasks,
    ...
    def _min_q_exp(self, obs, actions):
        #print(obs.shape,actions.shape)
        q1 = self.qf1_exp(obs, actions)
        q2 = self.qf2_exp(obs, actions)
        min_q = torch.min(q1, q2)
        return min_q
    ...
    def _take_step(self, indices, context,context_unbatched):
        ...
        # Q and V networks
        # encoder will only get gradients from Q nets
        q1_pred = self.qf1_exp(obs, actions,)
        q2_pred = self.qf2_exp(obs, actions)
...
class ExpSACSimpleOnline(ExpAlgorithmSimpleOnline):
    def __init__(
            self,
            env,
            train_tasks,
    ...
    def _take_step(self, indices, context,context_unbatched):
        ...
        obs, actions, rewards, next_obs, terms = context_unbatched
        policy_outputs, rew_entropy = self.exploration_agent(obs, context)
        path_rew = 0#ExpAlgorithm.sample_eval(self,indices,context)
        path_rew = 0#torch.Tensor.repeat(path_rew,1,rew_entropy.shape[1],1)
        rew_entropy = torch.unsqueeze(rew_entropy,2)
        #print(rew_entropy.shape,path_rew.shape)
