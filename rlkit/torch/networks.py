...
class AttentionEncoder(FlattenMlp):
    '''
    encode context via recurrent network
    '''
    ...
    def forward(self, in_, return_preactivations=False):
        ...
        for j in range(seq):
            ...
            #print(self.hidden.permute(1,2,0).shape)
            weights = torch.bmm(m,self.hidden.permute(1,2,0))
            weights = torch.softmax(weights,dim=1)
            r = torch.bmm(torch.transpose(weights,1,2),m)
            out1, (hn, cn) = self.lstm(r, (self.hidden, torch.zeros(self.hidden.size()).to(ptu.device)))
            self.hidden = hn
        # take the last hidden state to predict z
        out = out[:, -1, :]
...
class SnailEncoder(FlattenMlp):
    def __init__(self,
        ...
        #self.lstm = nn.LSTM(self.hidden_dim, self.hidden_dim, num_layers=1, batch_first=True)
        layer_count = 7
        self.TC1 = TCBlock(self.hidden_dim,128,16)
        self.atten1 = AttentionBlock(self.hidden_dim+16*layer_count,32,32)
        self.TC2 = TCBlock(self.hidden_dim+16*layer_count+32,128,16)
